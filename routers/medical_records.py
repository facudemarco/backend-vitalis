...
def _validate_required_columns(db, table_name: str, data_dict: dict) -> None:
    """
    Report all required MySQL columns missing from a submitted medical-record section.
    IDs and foreign keys generated by the API are included in data_dict before validation.
    """
    required_columns = db.execute(
        text("""
            SELECT COLUMN_NAME
    ...
    if missing:
        raise HTTPException(
            status_code=422,
            detail={
                "message": "Completa los campos obligatorios de la ficha médica.",
                "fields": missing
            }
        )
...
async def create_medical_record(
    ...
    try:
        ...
        def insert_sub_table(table_name: str, data_dict: dict, inject_medical_record_id: bool = True):
            ...
            _validate_required_columns(db, table_name, data_dict)
        ...
        if request_model.medical_record_data:
             ...
             if data_dict:
                 ...
                 _validate_required_columns(db, "medical_record_data", data_dict)
